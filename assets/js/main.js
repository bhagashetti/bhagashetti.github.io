...
// Apply the dark theme by default or based on user's previous selection
if (selectedTheme) {
...
themeButton.addEventListener("click", () => {
  ...
  // Save the user's theme and icon preference
...
myform.addEventListener("submit", function (event) {
  ...
  const service_id = "service_owx07lr";
  const template_id = "template_f1xx1hv";

  // Get the submit button and update its text to "Sending..."
  ...
  emailjs.sendForm(service_id, template_id, myform).then(
    ...
    function (err) {
      alert("Email sending failed! Response: " + JSON.stringify(err));
      submitButton.textContent = "Send Message"; // Reset button text
    }
  );
});
